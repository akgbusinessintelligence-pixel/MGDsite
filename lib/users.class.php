<?php
    ...
    class Users
    {
        ...
        public function _addStaff($validate)
        {
            ...
            if (empty(Message::$msgs)) {
                ...
                $data = array(
                    ...
                    'lname' => $safe->lname,
                    'fname' => $safe->fname,
                    'userlevel' => ($safe->usertype == 'staff' ? 8 : ($safe->usertype == 'editor' ? 7 : 5)),
                    'hash' => $hash,
                    'active' => 'y',
                    'usertype' => $safe->usertype,
                );
                
                $last_id = Db::Go()->insert(self::aTable, $data)->run();
        ...
        public function quickMessage()
        {
            $validate = Validator::Run($_POST);
            $validate
                ->set("message", Lang::$word->CL_QMSG)->required()->string(true, true)
