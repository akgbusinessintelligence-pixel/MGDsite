<?php
    ...
    class Front
    {
        ...
        public function Index()
        {
            ...
            $sqlPop = "SELECT idx, nice_title, slug, thumb, year, price, sold FROM `" . Items::lTable . "` WHERE status = ? AND featured = ? ORDER BY created DESC LIMIT 0, " . App::Core()->featured;
            
            $tpl->popular = $core->show_popular ?
                Db::Go()->rawQuery($sqlPop, array(1, 1))->run() :
                null;
        ...
        public function Edit ($idx) {
            ...
            if (!$row = Db::Go()->select(Items::lTable)->where("idx", $idx, "=")->where("status", 1, "=")->where("user_id", App::Auth()->uid, "=")->first()->run()) {
                $tpl->title = Lang::$word->META_ERROR;
                $tpl->template = 'front/themes/' . $core->theme . '/404.tpl.php';
                if (DEBUG) {
                    Debug::AddMessage("warnings", '<i>ERROR</i>', "Invalid idx detected [front.class.php, ln.:" . __line__ . "] slug [" . $idx . "]", "session");
                }
            } else {
                $tpl->row = $row;
                $tpl->features = Db::Go()->select(Content::fTable)->orderBy("sorting", "ASC")->run();
                $tpl->fin = Utility::jSonToArray($tpl->row->features);
                $tpl->gallery = Db::Go()->select(Items::gTable)->where("listing_id", $tpl->row->id, "=")->orderBy("sorting", "ASC")->run();
                ...
                $tpl->template = 'front/themes/' . $core->theme . '/edit.tpl.php';
            }
        }
        ...
        public function addListing()
        {
            ...
            $validate
                ...
                ->set("ylink", Lang::$word->LST_YTUBE)->string()
                ->set("lplate", Lang::$word->LST_LPLATE)->string()
            ...
            if (empty(Message::$msgs)) {
                ...
                $idata = array(
                    ...
                    'feature_name' => Db::Go()->select(Content::fTable, array("name"))->where("id", $safe->features, "IN")->orderBy("sorting", "ASC")->run('json'),
                    'price' => $safe->price,
                    'year' => $safe->year,
                    'mileage' => $safe->mileage,
                    'special' => 0,
                ...
                Db::Go()->insert(Items::liTable, $idata)->run();
                ...
                File::copyDirectory(UPLOADS . "/listings/pics___temp___" . $temp_id . "/", UPLOADS . "/listings/pics" . $last_id . "/");
                File::deleteRecursive(UPLOADS . "/listings/pics___temp___" . $temp_id, true);
        ...
        public function updateItem() {
            ...
            if(!$item = Db::Go()->select(Items::lTable, array("nice_title"))->where("id", Filter::$id, "=")->where("user_id", App::Auth()->uid, "=")->first()->run()) {
                Message::$msgs['id'] = Lang::$word->SYSTEM_ERR1;
            }
            
            $thumb = File::upload("thumb", Items::MAXIMG, "png,jpg,jpeg,webp");
            $file = File::upload("file", Items::MAXFILE, Items::FILES);
            $safe = $validate->safe();
            ...
            if (empty(Message::$msgs)) {
                $data = array(
                    ...
                    'body' => $safe->body ? Url::in_url($safe->body) : "",
                    'modified' => Db::toDate(),
                    'gallery' => Db::Go()->select(Items::gTable)->where("listing_id", Filter::$id, "=")->orderBy("sorting", "ASC")->run('json'),
                );
                $datax = array();
                //process thumb
                ...
                if (!empty($_FILES['thumb']['name'])) {
                    ...
                    try {
                        ...
                        $img->bestFit(Items::THUMBW, Items::THUMBH)->save($thumbpath . 'thumbs/' . $tresult['fname']);
                    } catch (exception $e) {
                        Debug::AddMessage("errors", '<i>Error</i>', $e->getMessage(), "session");
                    }
                ...
                Db::Go()->update(Items::lTable, array_merge($data, $datax))->where("id", Filter::$id, "=")->where("user_id", App::Auth()->uid, "=")->run();
                
                $message = Message::formatSuccessMessage($item->nice_title, Lang::$word->LST_UPDATED);
                Message::msgReply(Db::Go()->affected(), 'success', $message);
                
                //Add to listings info
                $make_name = Db::Go()->select(Content::mkTable, array("name"))->where("id", $data['make_id'], "=")->one()->run();
                $category = Db::Go()->select(Content::ctTable, array("name", "slug"))->where("id", $data['category'], "=")->first()->run();
                $location_name = Db::Go()->select(Content::lcTable)->where("id", $data['location'], "=")->first()->run();
                ...
                Db::Go()->update(Items::liTable, $idata)->where("listing_id", Filter::$id, "=")->run();
        ...
        public function contactSeller()
        {
            ...
            $validate
                ...
                ->set("message", Lang::$word->MESSAGE)->required()->string(true, true)->min_len(10)->max_len(200)
                ->set("email", Lang::$word->EMAIL)->required()->email()
                ->set("agree", Lang::$word->PRIVACY)->required()->numeric()
        ...
        public function contact()
        {
            $validate = Validator::run($_POST);
            $validate
                ->set("name", Lang::$word->EMN_NLN)->required()->string()->min_len(2)->max_len(60)
                ->set("notes", Lang::$word->MESSAGE)->required()->string(true, true)->min_len(10)->max_len(200)
                ->set("email", Lang::$word->EMAIL)->required()->email()
                ->set('captcha', Lang::$word->CAPTCHA)->required()->numeric()->equals(App::Session()->get('wcaptcha'))->exact_len(5);
            
            $safe = $validate->safe();
            if (empty(Message::$msgs)) {
                $core = App::Core();
                $html_message = Utility::getSnippets(BASEPATH . 'mailer/' . $core->lang . '/Contact_Request.tpl.php');
                $subject = str_replace("[COMPANY]", $safe->name, Lang::$word->HOME_MSG_CREQ);
                $mailer = Mailer::sendMail();
