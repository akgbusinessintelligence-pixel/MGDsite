    ...
    class Items
    {
        ...
        public function processItem()
        {
            ...
            $validate
                ...
                ->set("color_e", Lang::$word->LST_INTC)->required()->string()
                ->set("color_i", Lang::$word->LST_EXTC)->required()->string()
        ...
        public function _addItem($validate)
        {
            ...
            if (empty(Message::$msgs)) {
                ...
                $data = array(
                    ...
                    'torque' => $safe->torque,
                    'price' => $safe->price,
                    'price_sale' => empty($safe->price_sale) ? "0.00" : $safe->price_sale,
                    'color_e' => $safe->color_e,
                    'color_i' => $safe->color_i,
                    'doors' => $safe->doors,
                    'fuel' => $safe->fuel,
                ...
                File::copyDirectory(UPLOADS . "/listings/pics" . $temp_id . "/", UPLOADS . "/listings/pics" . $last_id . "/");
                File::deleteRecursive(UPLOADS . "/listings/pics" . $temp_id, true);
        ...
        public function Search()
        {
            ...
            if (array_intersect_key($qs, $required)) {
                ...
                if (array_key_exists('year', $qs)) {
                    if ($year = Validator::sanitize($qs['year'], "int", 4)) {
                        $and .= " AND l.year = $year";
                    }
                }
                if (array_key_exists('fuel', $qs)) {
                    if ($fuel = Validator::sanitize($qs['fuel'], "int", 3)) {
                        $and .= " AND l.fuel = $fuel";
                    }
                }
                if (array_key_exists('price', $qs)) {
        ...
        public static function doCalc()
        {
            ...
            $sql = "
            SELECT c.image, li.category_name, li.category_slug, COUNT(li.id) as total
              FROM `" . Items::liTable . "` as li
              LEFT JOIN `" . Content::ctTable . "` as c on li.category_slug = c.slug
              WHERE li.lstatus = ?
              GROUP BY li.category_slug
            ";
            $category = Db::Go()->rawQuery($sql, array(1))->run("json");
            ...
            $ids = Db::Go()->select(Items::lTable, array("GROUP_CONCAT(make_id) as mkids, GROUP_CONCAT(model_id) as mdids"))->where('status', 1, '=')->first()->run();
            $makes = $ids ? Db::Go()->select(Content::mkTable, array("id", "name"))->where("id", explode(",", $ids->mkids), "IN")->groupBy("id")->run("json") : "[]";
            $models = $ids ? Db::Go()->select(Content::mdTable, array("id", "name"))->where("id", explode(",", $ids->mdids), "IN")->groupBy("id")->run("json") : "[]";
