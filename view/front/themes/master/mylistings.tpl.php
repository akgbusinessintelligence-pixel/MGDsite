   ...
   <div class="wojo-grid">
      ...
      <div class="row gutters">
         ...
         <div class="columns auto phone-100">
            <a href="<?php echo Url::url("/dashboard", "new"); ?>" class="wojo primary fluid button">
               <i class="icon plus"></i><?php echo Lang::$word->LST_ADD; ?></a>
         </div>
      </div>
      <?php if (!$this->data): ?>
         ...
         <div class="wojo mason three">
            <?php foreach ($this->data as $row): ?>
               <div class="wojo attached basic card" id="item_<?php echo $row->id; ?>">
                  <a href="<?php echo Url::url("/dashboard/edit", $row->idx); ?>" class="wojo top left small attached spaced primary circular inverted icon button">
                  ...
                  <div class="content">
                     <div class="center aligned">
                        ...
                        <div class="wojo horizontal divided list">
                           ...
                           <div class="item"><?php echo $row->status? '<i class="icon small positive check"></i>' : '<i class="icon negative small circle slash"></i>'; ?><?php echo Lang::$word->ACTIVE; ?></div>
                        ...
                        <div class="columns">
                           <?php if ($row->sold): ?>
                              <a id="icon_sold_<?php echo $row->id; ?>"
                                 data-set='{"option":[{"iaction":"notSold", "id":<?php echo $row->id; ?>}], "url":"/controller.php", "complete":"replaceWith", "parent":"#icon_sold_<?php echo $row->id; ?>"}'
                                 class="wojo small primary inverted button iaction">
                                 <i class="icon check"></i><?php echo Lang::$word->SOLD; ?></a>
                           <?php else: ?>
                              <a id="icon_sold_<?php echo $row->id; ?>"
                                 ...
                                 class="wojo small primary inverted button iaction">
                                 <i class="icon circle slash"></i><?php echo Lang::$word->SOLD_M; ?></a>
                           <?php endif; ?>
                        </div>
                        <div class="columns auto">
                           <a data-set='{"option":[{"delete": "deleteListing","title": "<?php echo Validator::sanitize($row->title, "chars"); ?>","id": <?php echo $row->id; ?>}],"action":"delete","parent":"#item_<?php echo $row->id; ?>"}'
                              class="wojo small negative inverted button data">
                              <i class="icon trash"></i><?php echo Lang::$word->DELETE; ?></a>
                        </div>
                     </div>
                  </div>
               </div>
