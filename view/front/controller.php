<?php
    ...
    switch ($pAction):
        ...
        case "processImages":
            ...
            $filedir = isset($_POST['mode']) ? UPLOADS . '/listings/pics' . Filter::$id : UPLOADS . '/listings/pics___temp___' . Filter::$id;
            $maxSize = 8388608;
            $exts = ['png', 'jpg', 'jpeg'];
            $json = [];
            ...
            for ($x = 0; $x < $num_files; $x++):
                ...
                $img = isset($_POST['mode']) ? UPLOADURL . '/listings/pics' . Filter::$id : UPLOADURL . '/listings/pics___temp___' . Filter::$id;
                $mode = isset($_POST['mode']) ? ',"mode":"edit"' : null;
                
                $json['type'] = "success";
                $json['html'][$x] = '
                <div class="columns" id="item_' . $last_id . '" data-id="' . $last_id . '">
                  <div class="wojo attached simple fitted segment center aligned"><img src="' . $img . '/thumbs/' . $name . '" alt="" class="wojo rounded grab image">
                    <div class="wojo buttons middle attached">
                      ...
                      <a data-set=\'{"option":[{"delete": "deleteGallery","title": "-/-","id": ' . $last_id . '}],"action":"delete","parent":"#item_' . $last_id . '" ' . $mode . '}\' class="wojo mini icon negative button data"><i class="icon trash"></i></a>
                    </div>
                  </div>
    ...
    switch ($iAction):
        ...
        case "makeList":
            $html = "";
            if (isset($_POST['type'])):
                $result = Db::Go()->select(Content::mdTable, array("id", "name"))->where("make_id", Filter::$id, "=")->run();
        ...
        case "popCategory":
            $html = "";
            $sql = "SELECT idx, nice_title, slug, thumb, year, price, sold FROM `" . Items::lTable . "` WHERE status = ? AND featured = ? AND category = ? ORDER BY created DESC LIMIT 0, " . App::Core()->featured;
            if ($result = Db::Go()->rawQuery($sql, array(1, 1, Filter::$id))->run())
                :
                foreach ($result as $row):
                    $html .= '
                    ...
                    if ($row->sold):
                        $html .= '<div class="wojo bookmark">' . Lang::$word->SOLD . '</div>';
                    endif;
                    $html .= '
        ...
        case "sold":
            ...
            if (Db::Go()->update(Items::lTable, array("sold" => 1))->where("id", Filter::$id, "=")->run()):
                $json['type'] = "success";
                $json['html'] = '<a class="wojo small primary inverted button iaction"><i class="icon check"></i>' . Lang::$word->SOLD . '</a>';
                print json_encode($json);
            endif;
            break;
        ...
        case "notSold":
            ...
            if (Db::Go()->update(Items::lTable, array("sold" => 0))->where("id", Filter::$id, "=")->run()):
                $json['type'] = "success";
                $json['html'] = '<a class="wojo small primary inverted button iaction"><i class="icon circle slash"></i>' . Lang::$word->SOLD_M . '</a>';
                print json_encode($json);
            endif;
            break;
        /* == Sort Gallery == */
    ...
    switch ($delete):
        ...
        case "deleteListing":
            ...
            if($row = Db::Go()->select(Items::lTable, array("thumb", "file"))->where("id", Filter::$id, "=")->where("user_id", App::Auth()->uid, "=")->first()->run()) {
