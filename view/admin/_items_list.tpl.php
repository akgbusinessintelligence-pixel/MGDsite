...
<div class="row gutters align middle">
   ...
   <div class="columns auto phone-100">
      <?php if (Auth::hasPrivileges('add_items')): ?>
         <a href="<?php echo Url::url("/admin/items", "new"); ?>" class="wojo small secondary button">
            <i
   ...
   <form method="post" id="wojo_form" action="<?php echo Url::url(Router::$path); ?>" name="wojo_form">
      <div class="row align center middle gutters">
         ...
         <div class="columns auto phone-hide">
            <a href="<?php echo Url::url(Router::$path); ?>" class="wojo icon button">
         ...
         <?php foreach ($this->data as $row): ?>
            <tr id="item_<?php echo $row->id; ?>">
               ...
               <td><b><?php echo $row->title; ?></b> (<?php echo $row->year; ?>) <br />
                  <small><?php echo Lang::$word->BY; ?>:
                     <?php if (Auth::hasPrivileges('edit_members')): ?>
                        <?php if ($row->is_owner): ?>
                           admin
                        <?php else: ?>
                           <a href="<?php echo Url::url("/admin/members/edit", $row->user_id); ?>"><?php echo $row->username; ?></a>
                        <?php endif; ?>
                     <?php else: ?>
                        <?php echo ($row->is_owner)? 'admin' : $row->username; ?>
                     <?php endif; ?>
                  </small>
                  <br />
                  #: <b><?php echo $row->stock_id; ?></b>
                  <br />
                  <?php echo Lang::$word->LST_PRICE; ?>: (<?php echo Utility::formatMoney($row->price); ?>)
                  <small
                    class="wojo text negative"><?php echo Utility::formatMoney($row->price_sale); ?></small>
                  <br />
                  ...
                  <?php else: ?>
                     <div class="wojo negative label"><?php echo Lang::$word->EXPIRED; ?>:
                        <span
                          class="detail"><?php echo Date::dodate("long_date", $row->expire); ?></span>
                     </div>
                  <?php endif; ?></td>
               ...
               <td class="auto">
                  <a data-wdropdown="#dropdown-itemData" class="wojo white small icon button" id="itemData">
                  ...
                  <div class="wojo small dropdown pointing menu top-left" id="dropdown-itemData">
                     ...
                     <a
                       ...
                       class="item iaction">
                        <i id="icon_featured_<?php echo $row->id; ?>"
                     ...
                     <a
                       ...
                       class="item iaction">
                        <i id="icon_sold_<?php echo $row->id; ?>"
                  ...
                  <a href="<?php echo Url::url(Router::$path, "stats/" . $row->id); ?>"
                     class="wojo icon dark small inverted button">
                     <i class="icon pie chart fill"></i>
                  </a>
                  ...
                  <a href="<?php echo Url::url(Router::$path, "images/" . $row->id); ?>"
                     class="wojo icon positive small inverted button">
                     <i class="icon images"></i>
                  </a>
                  <?php if (Auth::hasPrivileges('edit_items')): ?>
                     <a href="<?php echo Url::url(Router::$path, "edit/" . $row->id); ?>"
                        class="wojo icon primary small inverted button">
