    ...
    switch ($trash):
        ...
        case "trashUser":
            if ($row = Db::Go()->select(Users::mTable)->where("id", Filter::$id, "=")->first()->run()):
                ...
                if ($location = Db::Go()->select(Content::lcTable)->where("user_id", Filter::$id, "=")->run()):
                    $ldata = array('type' => "location", 'dataset' => json_encode($location));
                    Db::Go()->insert(Core::txTable, $ldata)->run();
                    Db::Go()->delete(Content::lcTable)->where("user_id", $location->id, "=")->run();
                endif;
            endif;
